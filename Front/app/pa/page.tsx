...
const PacientePage = () => {
    ...
    const examenesFiltrados =
        examenes?.filter((examen) => {
            const cumpleTipoExamen =
                !filtros.tipoExamen ||
                examen.tipo_examen === filtros.tipoExamen;
    ...
    const descargarExamenCompleto = async (examen: any) => {
        try {
            ...
            alert(
                'Error al descargar el examen. Por favor, inténtelo de nuevo.'
            );
        }
    };
                        ...
                        <div className='space-y-4'>
                            {examenesFiltrados.length === 0 ? (
                                ...
                                examenesFiltrados.map((examen) => {
                                    const examenId =
                                        examen.examen_id.toString();
                                    const abierto = abiertas.has(examenId);
                                    const totalMuestras =
                                        examen.muestras?.length || 0;
