...
async function dashboard(_req, res) {
  try {
    const [monthlyRows] = await sequelize.query(`
      SELECT
        EXTRACT(YEAR FROM fecha_diagnostico)::INT AS year,
        EXTRACT(MONTH FROM fecha_diagnostico)::INT AS month,
        COUNT(*)::INT AS total
      FROM episodio
    ...
    const [distributionRows] = await sequelize.query(`
      SELECT
        tipo_fractura AS tipo,
        COUNT(*)::INT AS total
      FROM episodio
      WHERE tipo_fractura IS NOT NULL
      GROUP BY tipo_fractura
      ORDER BY tipo_fractura;
    `);
    ...
    // Asegura orden cronológico en series por cada año
    for (const yearKey of Object.keys(series)) {
      series[yearKey].sort((a, b) => MONTH_LABELS.indexOf(a.mes) - MONTH_LABELS.indexOf(b.mes));
    }
