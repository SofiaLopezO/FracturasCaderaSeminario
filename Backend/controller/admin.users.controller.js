  ...
  try {
    ...
    try {
      ...
      if (payload.user) {
        ...
        const cargoValido = ['FUNCIONARIO', 'TECNOLOGO', 'INVESTIGADOR'].includes(cargoProfesional) ? cargoProfesional : 'FUNCIONARIO';
      ...
      await logRegistro(req, `CREAR_USUARIO: ${correo}, role=${role}`, user.id);
...
async function addRoleToUser(req, res) {
  try {
    ...
    await logRegistro(req, `ASIGNAR_ROL: user_id=${userId}, role=${role}`, userId);
...
async function removeRoleFromUser(req, res) {
  try {
    ...
    await logRegistro(req, `QUITAR_ROL: user_id=${userId}, role=${role}`, userId);
...
const ProfessionalProfile =
  models.ProfessionalProfile || models.professional_profile || models.Profile || models.profile;
...
const updateUserProfile = async (req, res) => {
  try {
    ...
    await logRegistro(req, `ACTUALIZAR_PERFIL_USUARIO: user_id=${id}`, id);
...
async function deactivateUser(req, res) {
  try {
    ...
    const user = await models.User.findByPk(id);
    if (!user) return res.status(404).json({ error: 'Usuario no encontrado' });
    if (user.deleted_at) return res.status(200).json({ ok: true, message: 'Ya estaba desactivado' });

    user.is_active = false;
    user.deleted_at = new Date();
    await user.save();

    await logRegistro(req, `DESACTIVAR_USUARIO: user_id=${id}`, id);
    return res.json({ ok: true, message: 'Usuario desactivado', id });
  } catch (err) {
...
async function reactivateUser(req, res) {
  try {
    ...
    await logRegistro(req, `REACTIVAR_USUARIO: user_id=${id}`, id);
    return res.json({ ok: true, message: 'Usuario reactivado', id });
  } catch (err) {
