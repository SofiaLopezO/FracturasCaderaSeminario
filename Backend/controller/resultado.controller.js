...
async function list(req, res) {
    try {
        const rows = await models.Resultado.findAll({
...
async function getOne(req, res) {
    try {
        ...
        const row = await models.Resultado.findByPk(id);
    ...
    try {
        ...
        if (!parametro || valor === undefined)
            return res
                .status(400)
                .json({
                    error: 'parametro, valor y fecha_resultado son obligatorios',
                });
        ...
        if (paciente_id) {
            ...
            if (!ultimoEpisodio) {
                return res
                    .status(400)
                    .json({
                        error: 'No se encontró un episodio para el paciente_id proporcionado',
                    });
            }
        ...
        if (muestra_id !== undefined && muestra_id !== null) {
            const m = await models.Muestra.findByPk(muestra_id);
            if (!m)
                return res.status(400).json({ error: 'muestra_id no existe' });
        }
...
async function update(req, res) {
    try {
        ...
        if (fecha_resultado !== undefined)
            row.fecha_resultado = fecha_resultado
                ? parseDate(fecha_resultado)
                : row.fecha_resultado;
        if (muestra_id !== undefined) {
            if (muestra_id === null) row.muestra_id = null;
            else {
                const m = await models.Muestra.findByPk(muestra_id);
                if (!m)
                    return res
                        .status(400)
                        .json({ error: 'muestra_id no existe' });
                row.muestra_id = muestra_id;
            }
        }
        if (examen_id !== undefined) {
            if (examen_id === null) row.examen_id = null;
            else {
                const ex = await models.Examen.findByPk(examen_id);
                if (!ex)
                    return res
                        .status(400)
                        .json({ error: 'examen_id no existe' });
        ...
        try {
            ...
            console.error(
                'recalculo riesgo post-resultado update',
                recalcError
            );
        }
        try {
            ...
            console.error(
                'alerta laboratorio post-resultado update',
                alertError
            );
        }
        res.json(row);
