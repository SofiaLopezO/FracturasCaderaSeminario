...
beforeEach(() => {
  ...
  fakeModels = {
    ParametroLab: {
      findByPk: vi.fn(), // devolverá { ref_min, ref_max, unidad, nombre }
    },
    Alerta: {
      findOne: vi.fn(),
...
describe('labAlertService.syncAlertForResultado', () => {
  it('crea alerta LAB cuando valor está por SOBRE el máximo', async () => {
    fakeModels.ParametroLab.findByPk.mockResolvedValue({
      ref_min: 3.5, ref_max: 5.1, unidad: 'mmol/L', nombre: 'Potasio',
    });
    fakeModels.Alerta.findOne.mockResolvedValue(null);

    const out = await syncAlertForResultado(resultadoBase, { models: fakeModels });
  ...
  it('actualiza alerta existente si sigue fuera de rango', async () => {
    ...
    const existing = {
      tipo: 'LAB', severidad: 'MEDIA', mensaje: 'old',
      episodio_id: 123, indicador_id: null, activa: true, resuelta_en: null,
      save: vi.fn(async () => {}),
      get: () => ({ tipo: 'LAB', severidad: 'ALTA', mensaje: 'x', episodio_id: 123, activa: true }),
    };
  ...
  it('resuelve alerta si el valor vuelve a rango', async () => {
    ...
    const out = await syncAlertForResultado(resultadoOk, { models: fakeModels });
    expect(existing.save).toHaveBeenCalled();
  ...
  it('no hace nada si no hay referencia (min y max nulos) o valor no numérico', async () => {
    // sin referencia
    fakeModels.ParametroLab.findByPk.mockResolvedValue({
      ref_min: null, ref_max: null, unidad: '', nombre: 'X',
    });
    const out1 = await syncAlertForResultado(resultadoBase, { models: fakeModels });
    expect(out1).toBeNull();

    // referencia sí, pero valor inválido
    fakeModels.ParametroLab.findByPk.mockResolvedValue({
      ref_min: 1, ref_max: 2, unidad: '', nombre: 'X',
    });
    const out2 = await syncAlertForResultado({ ...resultadoBase, valor: 'N/A' }, { models: fakeModels });
