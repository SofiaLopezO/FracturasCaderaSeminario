#!/usr/bin/env node
/**
 * Importa TIPOMUESTRA.csv a public.tipo_muestra (UPSERT por id o insert por nombre).
 * Columnas esperadas: id, nombre, descripcion (en cualquier orden)
 * Uso:
 *   node scripts/import_tipomuestra.js ./scripts/TIPOMUESTRA.csv --preview
...
const INPUT = process.argv[2] || path.resolve(process.cwd(), "scripts", "TIPOMUESTRA.csv");
const SCHEMA = "public";
const TABLE = "tipo_muestra";
...
(async () => {
  ...
  try {
    // Limpia estado por si el pool reutiliza una conexión "aborted"
    try { await cx.query("ROLLBACK"); } catch (_) {}

    // DDL en su propia transacción
    ...
    await cx.query("COMMIT");
    console.log(`Importación tipo_muestra: OK=${ok}  Errores=${bad}`);
  } catch (err) {
    try { await cx.query("ROLLBACK"); } catch (_) {}
